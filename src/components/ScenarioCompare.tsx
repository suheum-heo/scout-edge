...
function winnerStyle(aVal: number, bVal: number, side: 'a' | 'b'): string {
  if (aVal === bVal) return 'text-slate-500'
  const winner = aVal > bVal ? 'a' : 'b'
  return winner === side ? 'text-emerald-400 font-bold' : 'text-slate-400 dark:text-slate-500'
}

function DimRow({ dimA, dimB }: { dimA: ScenarioDimension; dimB: ScenarioDimension }) {
  return (
    <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-center py-1.5 border-b border-slate-200/30 dark:border-slate-700/30 last:border-0">
      <div className="text-right">
        <span className={`text-sm ${winnerStyle(dimA.scenarioScore, dimB.scenarioScore, 'a')}`}>
          {dimA.scenarioScore}/10
        </span>
      </div>
      <div className="text-center text-slate-400 dark:text-slate-500 text-[10px] w-24">{dimA.label}</div>
...
export default function ScenarioCompare({ a, b }: Props) {
  ...
  return (
    <div className="border border-blue-500/20 bg-slate-50 dark:bg-slate-800/60 rounded-xl p-4 mb-4">
      ...
      {/* Overall scores */}
      <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-center pt-2 border-t border-slate-200/50 dark:border-slate-700/50">
