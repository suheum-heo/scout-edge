 ...
 * IDs are football-data.org v4 team IDs.
 * Falls back to the football-data.org API for obscure clubs not listed here.
 */
  ...
  return results
    ...
    .map(({ team }) => ({
      team: {
        ...
        logo: `https://crests.football-data.org/${team.id}.png`,
      },
      venue: { name: '', city: '' },
    }))
}
