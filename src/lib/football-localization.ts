...
export const MANUAL_NAME_GLOSSARY: Record<LanguageCode, Partial<Record<EntityType, Record<string, string>>>> = {
  ...
  ja: {
    ...
    player: {
      ...
      'Guglielmo Vicario': 'グリエルモ・ヴィカーリオ',
      'Dejan Kulusevski': 'デヤン・クルシェフスキ',
      ...
      'Christian Romero': 'クリスティアン・ロメロ',
      'João Palhinha': 'ジョアン・パリーニャ',
      'Pape Matar Sarr': 'パペ・マタル・サール',
      'Pape Sarr': 'パペ・サール',
      'Yves Bissouma': 'イヴ・ビスマ',
...
const TERM_TRANSLATIONS: Record<LanguageCode, TermDictionary> = {
  ...
  ko: {
    ...
    'passing accuracy': '패스 정확도',
    'positional awareness': '위치 선정',
    'off-ball movement': '오프더볼 움직임',
    'press resistance': '압박 저항성',
    'pressure resistance': '압박 저항성',
    ...
    'build-up': '빌드업',
    'build up': '빌드업',
    'recovery pace': '수비 복귀 속도',
    'aerial ability': '공중볼 능력',
    'ball carrying': '볼 운반',
    'chance creation': '찬스 창출',
    'finishing': '마무리',
    ...
    'technical sweeper keeper': '기술형 스위퍼 키퍼',
    'elite technical sweeper-keeper': '엘리트 기술형 스위퍼 키퍼',
    'elite technical sweeper keeper': '엘리트 기술형 스위퍼 키퍼',
    'ball-playing centre-back': '볼플레잉 센터백',
    'ball-playing center-back': '볼플레잉 센터백',
    ...
    'deep-lying playmaker': '후방 플레이메이커',
    'anchor midfielder': '앵커 미드필더',
    'pressing forward': '압박형 공격수',
  ...
  ja: {
    ...
    'speed': 'スピード',
    'pass accuracy': 'パス精度',
    'passing accuracy': 'パス精度',
    'positional awareness': 'ポジショニング認識',
    'off-ball movement': 'オフザボールの動き',
    'press resistance': 'プレス耐性',
    'pressure resistance': 'プレス耐性',
    'pressing intensity': 'プレス強度',
    'dribbling': 'ドリブル',
    'ball distribution': '配球',
    'ball retention': 'ボール保持',
    'distribution accuracy': '配球精度',
    ...
    'ball-winning': 'ボール奪取',
    'ball winning': 'ボール奪取',
    'progressive passing': '前進パス',
    'defensive coverage': '守備範囲',
    'tackling': 'タックル',
    'crossing': 'クロス',
    'work rate': '運動量',
    'physicality': 'フィジカル',
    ...
    'technical sweeper keeper': 'テクニカル・スイーパーキーパー',
    'elite technical sweeper-keeper': 'エリート技術型スイーパーキーパー',
    'elite technical sweeper keeper': 'エリート技術型スイーパーキーパー',
    ...
    'technical left winger': '技術型左ウイング',
    'technical right winger': '技術型右ウイング',
    'balanced midfielder': 'バランス型ミッドフィルダー',
    'wide creator': 'ワイドクリエーター',
    'box-to-box midfielder': 'ボックス・トゥ・ボックスMF',
    'deep-lying playmaker': 'ディープライイング・プレーメーカー',
    'anchor midfielder': 'アンカー型MF',
    'pressing forward': 'プレス型フォワード',
...
const LOCALIZED_TERM_NORMALIZATIONS: Record<LanguageCode, TermDictionary> = {
  en: {},
  ko: {
    '압박 저항': '압박 저항성',
    '볼 배급 정확도': '배급 정확도',
...
function localizeString(
  ...
  const localizedNormalizations = LOCALIZED_TERM_NORMALIZATIONS[language] || {}
  for (const [variant, replacement] of Object.entries(localizedNormalizations).sort((left, right) => right[0].length - left[0].length)) {
    localized = localized.replace(buildTermPattern(variant), replacement)
  }
