...
import { enrichTMPlayerIdentity, checkTMProxyHealth } from '@/lib/transfermarkt'
...
function isPositionallyEligible(target: TransferTarget, gapPositionCode: string, gapPosition: string): boolean {
  ...
  // Check both position string and positionCode: Claude may return "LWB"/"RWB" in either field.
  const WING_BACK_RE = /wing.?back|wingback|\blwb\b|\brwb\b/i
  const isWingBackGap = WING_BACK_RE.test(gapPosition) || WING_BACK_RE.test(gapPositionCode)
    ...
    for (let attemptIdx = 0; attemptIdx < 2; attemptIdx++) {
      ...
      if (!tmProxyHealthy) {
        ...
        filtered = targets.filter((t) => {
          ...
          return !clubNorm.includes(teamNorm) && !teamNorm.includes(clubNorm)
        })
        console.warn(`[recommendations] degraded mode: ${filtered.length} unverified results (TM proxy down)`)
        break
      }
