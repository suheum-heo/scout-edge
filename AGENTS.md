...
3. Manager auto-detected from coach name or Codex infers from name
4. Codex `analyzeSquadGaps()` → JSON gaps
5. User clicks gap → `/api/recommendations`: API Football candidates → TM enrichment → Codex ranks
6. Squad Fit tab → `/api/squad-fit`: Codex rates every squad player 1–10
...
- Tactical reasoning, fit scores, role analysis → Codex
...
- `src/lib/Codex.ts` — `analyzeSquadGaps`, `recommendPlayersForGap`, `analyzeSquadSystemFit`, `analyzeScenario`, `generateUndervaluedXI`, `buildManagerXI`, `analyzeTransferVerdict`
...
- Codex JSON responses parsed with `extractJSON()` + `sanitizeHomoglyphs()` (Cyrillic fix)
...
### ✅ V1 — Foundation
- Team search, squad fetch (football-data.org), squad gap analysis (Codex)
...
- Cyrillic homoglyph sanitization in Codex responses
...
- Scenario Builder: define OUT/IN players, Codex recalculates squad dimensions
...
- Pick any manager + budget → Codex builds ideal XI with archetype labels + TM enrichment
...
- Codex recalculates: role coverage, system fit, squad balance, age curve, depth
...
### ✅ V5 — Manager Identity Mode (`/build`)
- User picks manager + budget → Codex builds ideal XI from scratch
